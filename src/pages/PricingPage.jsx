            ...
            {homeownerPlans.map((plan, idx) => (
              <div
                ...
                className={`rounded-2xl p-8 border transition-all duration-300 ${
                  plan.featured
                    ? "border-primary bg-gradient-to-br from-teal-900 to-teal-800 text-white shadow-2xl transform md:scale-105"
                    : "border-teal-200 bg-white hover:border-primary/50"
                }`}
              >
                {plan.featured && (
                ...
                <h3 className={`text-2xl font-semibold mb-2 ${plan.featured ? "text-white" : "text-teal-900"}`} style={{ fontFamily: "'Playfair Display', serif" }}>
                  {plan.name}
                </h3>

                <div className="mb-6">
                  <div className={`text-4xl font-bold ${plan.featured ? "text-white" : "text-teal-900"}`}>
                    {plan.price}
                    <span className={`text-lg font-normal ${plan.featured ? "text-teal-300" : "text-slate-500"}`}>
                      {plan.period}
                    </span>
                  </div>
                  <p className={`text-sm mt-2 ${plan.featured ? "text-teal-300" : "text-teal-600"}`}>
                    {plan.description}
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  {plan.features.map((feature, fidx) => (
                    <div key={fidx} className="flex items-start gap-3">
                      ...
                      <span className={`text-sm ${plan.featured ? "text-teal-200" : "text-teal-600"}`}>
                        {feature}
                      </span>
                    </div>
                  ))}
        ...
        <div className="max-w-3xl mx-auto px-6 sm:px-10 lg:px-12">
          ...
          <div className="space-y-6">
            {[
              ...
              <Card key={idx} className="p-6 border-teal-200 hover:border-primary/50 transition-all">
                <h3 className="text-lg font-semibold text-teal-900 mb-2">{item.q}</h3>
