...
const STATUS_CONFIG = {
  pending:   { label: "Pending",   color: "border-copper-600/30 bg-copper-500/14 text-copper-100" },
    ...
    <Card className="result-card-surface border-0">
      <CardContent className="p-5 sm:p-6">
        <div className="booking-card-grid">
          ...
          {booking.notes && (
            <div className="info-tile border-copper-100 bg-copper-50/90">
                ...
                <div className={`flex items-center justify-between gap-4 rounded-[1rem] border border-border/70 bg-background/70 p-3 shadow-sm shadow-slate-950/10 ${!canOfferVideo ? "opacity-40" : ""}`}>
                  <div>
                    <Label className="text-sm font-medium flex items-center gap-1.5">
                      ...
                      Allow Video Call
                      {!canOfferVideo && (
                        <span className="ml-1 text-xs font-normal text-copper-400">(Pro plan)</span>
                      )}
                    </Label>
